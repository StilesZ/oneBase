...
class Menu extends AdminBase
{
    ...
    public function menuToView($menu_list = [], $child = 'child')
    {
        ...
        foreach ($menu_list as $menu_info) {
            ...
            if (!empty($menu_info[$child])) {
                ...
                $menu_view.=  "<li class='nav-item' menu_id='".$menu_info['id']."'>
                                 <a href='javascript:;' class='nav-link'>
                                 <i class='nav-icon $icon'></i><p>".$menu_info['name']."<i class='right fas fa-angle-left'></i></p>
                                 </a>
                                 <ul class='nav nav-treeview'>".$this->menuToView($menu_info[$child],  $child)."</ul>
                               </li>";


            } else {
                
                $icon = empty($menu_info['icon']) ? ' far fa-circle' : $menu_info['icon'];
                
                $url = url($menu_info['url']);
                
                $menu_view .= "<li class='nav-item' menu_id='".$menu_info['id']."'><a href='$url' class='nav-link'><i class='nav-icon $icon'></i><p>".$menu_info['name']."</p></a></li>";
            }
       }
    ...
    public function menuToCheckboxView($menu_list = [], $child = 'child')
    {
        ...
        foreach ($menu_list as $menu_info) {
            ...
            if($menu_info['id'] == 1) $checkbox_select = "checked='checked'";

            if (!empty($menu_info[$child])) {
                
                $menu_view.=  "<div class='card card-header admin-node-header'>
                                          <div class='card-header'><div class='checkbox'> <label>
                                                  <input class='rules_all' type='checkbox' name='rules[]' value='".$menu_info['id']."' $checkbox_select > <i class='fas $icon'></i>  ".$menu_info['name']."
                                              </label> </div></div>
                                    <div class='card-body'> ".$this->menuToCheckboxView($menu_info[$child],  $child)." </div>
                                </div>";
                
            } else {
                
                $menu_view.=    "<label class='admin-node-label'>  <input type='checkbox' name='rules[]' value='".$menu_info['id']."'  $checkbox_select > &nbsp;<i class='fas $icon'></i>  ".$menu_info['name']."  </label>";
            }
       }
    ...
    public function getCrumbsView()
    {
        ...
        $crumbs_view = "<ol class='breadcrumb float-sm-right'>";
      
        foreach (self::$crumbs as $menu_info) {
            
            $icon = empty($menu_info['icon']) ? 'fa-circle-o' : $menu_info['icon'];
            
            $crumbs_view .= "<li class='breadcrumb-item'><a> ".$menu_info['name']."</a></li>";
        }
